...
namespace AzDoc.Helpers
{
    public static class SFTPHelper
    {
            ...
            try
            {
                ...
                using (FileStream fs = File.OpenRead(fileLocalPath))
                {
                    ...
                    using (var client = new SftpClient(SFTPUser.Host, SFTPUser.Port, SFTPUser.UserName, SFTPUser.Password))
                    {
                        ...
                        string path = string.Concat(Helper.FileServerPath, '/' + Helper.GetFilePath());
            ...
            try
            {
                using (FileStream fs = File.OpenRead(fileLocalPath)) //Fayl oxunur
                {
                    using (var client = new SftpClient(SFTPUser.Host, SFTPUser.Port, SFTPUser.UserName, SFTPUser.Password)) //Sftp client hazırlanır
                    {
                        ...
                        client.Connect();
                        string path = string.Concat(Helper.EdocServerPath, '/' + Helper.GetFilePath());//Faylın sft-də hansı qovluğa yazılacağı yol hazırlanır: (Helper.FileServerPath: DMS_FILE/)/ (workingdirectory:2018/1/4/9)
        ...
        public static byte[] DownloadFileBuffer(string sftpFilePath)
        {
            try
            {
                using (var client = new SftpClient(SFTPUser.Host, SFTPUser.Port, SFTPUser.UserName, SFTPUser.Password))
                {
                    ...
                    string fileFullPath = string.Concat(Helper.FileServerPath, "/", sftpFilePath);
        ...
        public static byte[] DownloadFileBuffer(DocsFileInfoModel fileModel)
        {
            if (fileModel is null) return new byte[0];

            return DownloadFileBuffer(fileModel.FileInfoPath);
        }
        ...
        public static byte[] DownloadEDocFileBuffer(string sftpFilePath)
        {
            try
            {
                using (var client = new SftpClient(SFTPUser.Host, SFTPUser.Port, SFTPUser.UserName, SFTPUser.Password))
                {
                    ...
                    string fileFullPath = string.Concat(Helper.EdocServerPath, sftpFilePath).Replace(@"\", "/");

...
-- =============================================
CREATE PROCEDURE [serviceletters].[spCancelDocumentRequest]
@docId int,
@workPlaceId int,
...
IF @note IS NULL OR TRIM(@note)='' THROW 56000,'Imtina qeydi daxil edin',1;
...
BEGIN
 ...
 set @chiefWorkPlaceId=CASE WHEN  @directionworkPlaceId=@workPlaceId THEN @docCreatorWorkPlaceId ELSE @directionworkPlaceId END;--dbo.[fnGetDepartmentChief](@docCreatorWorkPlaceId);
...
and ex.ExecutorWorkplaceId=@workPlaceId
and ex.ExecutorReadStatus=0
and (ex.DirectionTypeId=15 or dv.VizaFromWorkflow=2)
AND dv.IsDeleted=0
...
IF EXISTS( SELECT TOP (1) 0 FROM DocOperationsLog dol WHERE dol.DocId=@docId AND dol.DirectionTypeId=4) 
BEGIN 
	  Insert into @OldViza(VizaId)
 select
 v.VizaId

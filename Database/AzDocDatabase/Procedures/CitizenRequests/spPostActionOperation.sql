...
CREATE PROCEDURE [citizenrequests].[spPostActionOperation] @actionId    INT, 
                                                          @docId       INT, 
                                                          @docTypeId   INT, 
                                                          @workPlaceId INT,
...
BEGIN TRY
      BEGIN TRANSACTION
        ...
        BEGIN
                        EXEC citizenrequests.spOperationSend 
                             @docId = @docId, 
        ...
        BEGIN
           exec citizenrequests.ReturnDocumentRequest
                            @docId=@docId,
                            @workPlaceId=@workPlaceId,
                            @note=@note;
           SET @result = 9;
        ...
        IF(@actionId = 13) -- Xarici məktubla cavabla
        BEGIN
            SET @result = 13;
        END;
                                      
        IF(@actionId = 14) -- Əlaqəli sənəd - Xarici məktub
        BEGIN
           SET @result = 14;
        END;
    COMMIT TRANSACTION;

...
CREATE PROCEDURE [smdo].[spPostActionOperation] @actionId    INT, 
                                                         @docId       INT, 
                                                         @docTypeId   INT, 
                                                         @workPlaceId INT, 
...
AS
    BEGIN
            ...
            IF(@actionId = 5) --Təsdiq et 
                BEGIN
                    IF(@documentStatusId IN(28, 16)) --Əgər vizadadirsa
                        BEGIN
                            EXEC [smdo].spOperationVisaAccept 
                                 @docId = @docId, 
                                 @docTypeId = @docTypeId, 
                                 @workPlaceId = @workPlaceId, 
                                 @note = @note, 
                    ...
                    END;
                        ELSE
                        BEGIN
                            EXEC [smdo].spOperationWhomAddress -- Tapshiriqlara gedir
                                 @docId = @docId, 
                                 @docTypeId = @docTypeId, 
                                 @workPlaceId = @workPlaceId, 
                                 @result = @result OUT;
            ...
            IF(@actionId = 6) --Imtina et
                BEGIN
                    EXEC [smdo].spCancelDocumentRequest 
                         @workPlaceId = @workPlaceId, 
                         @docId = @docId, 
                         @note = @note, 
                         @result = @result OUT;
            ...
            IF(@actionId = 7) --İmza et
                BEGIN
                    EXEC [smdo].spOperationConfirmSignature 
                         @docId = @docId, 
                         @docTypeId = @docTypeId, 
                         @workPlaceId = @workPlaceId, 
                         @note=@note,
            ...
            IF(@actionId = 9) --Geri
                BEGIN
                    EXEC [smdo].spReturnDocumentRequest 
                         @workPlaceId = @workPlaceId, 
                         @docId = @docId, 
                         @note = @note, 
                         @result = @result OUT;
